...
// JSON helper
function json(ResponseInterface $res, $data, int $status = 200): ResponseInterface {
...
// Health
$app->get('/health', fn($req,$res)=>json($res, ['ok'=>true]));
...
$app->post('/orders', function($req,$res) {
    ...
    Capsule::connection()->transaction(function() use ($product,$window,$needPieces,$email,$notes,$address,$qtyPacks,&$orderId,&$sessionUrl) {
        ...
        $lineAmount = (int) round($product->gross_price * 100);
...
$app->get('/orders/{id}', function($req,$res,$args){
    ...
    return json($res, ['order'=>$order,'items'=>$items]);
});

// Success/Cancel
$app->get('/success', fn($req,$res)=>json($res, ['ok'=>true]));
$app->get('/cancel',  fn($req,$res)=>json($res, ['ok'=>false]));
